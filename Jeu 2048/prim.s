...
;------ Transforme en position mémoire une position X,Y (PLOT)
CALCPOS
.(
 LDA #$BB
 STA posit+1
...
;----- Affiche à la position posit le texte spécifié en ($0A)
PRINT
.(
 LDA posit
 LDX posit+1
...
ETIQ
 ...
 ASL
 CLC
 ADC #$8  ; on multiplie par 4 et on ajoute 8 pour avoir la position à l'écran de la ligne X
 STA LIG
 TYA
 PHA
 STA COL
 ...
 ASL
 ADC #$B
 STA COL ; on multiplie par 6 et on ajoute 12 pour avoir la position de la colonne Y
 JSR CALCPOS ; La commande PRINT a maintenant la position à laquelle écrire
 PLA
 TAY
 PLA
 TAX
